...
const getProviderLogo = (name?: string, baseUrl?: string, platform?: string): string | null => {
  ...
  // 按名称精确匹配
  const byName = MODEL_PLATFORMS.find((p) => p.name === name);
  if (byName) return byName.logo;

  // 按名称模糊匹配（忽略大小写）
  const byNameLower = MODEL_PLATFORMS.find((p) => p.name.toLowerCase() === name?.toLowerCase());
  if (byNameLower) return byNameLower.logo;

  // 按 URL 匹配
  if (baseUrl) {
    const byUrl = MODEL_PLATFORMS.find(
      (p) => p.baseUrl && baseUrl.includes(p.baseUrl.replace(/^https?:\/\//, '').split('/')[0])
    );
...
const EditModeModal = ModalHOC<{ data?: IProvider; onChange(data: IProvider): void }>(
  ({ modalProps, modalCtrl, ...props }) => {
    ...
    const requiresApiKey = !isBedrock && !supportsNoApiKeyForOpenAICompatibleProvider(data?.baseUrl);
